...
int main() {
    ...
    CostNetFlow<int, int> nf(n * m + n + 2); int s = n * m + n, t = s + 1;
    r0p (i, n) nf.adde(s, i, 1, 0);
    r0p (i, m) r0p (j, n) {
        nf.adde(i * n + j + n, t, 1, 0);
        r0p (k, n) nf.adde(k, i * n + j + n, 1, (j + 1) * a[k][i]);
    }
    cout << fixed << setprecision(2) << double(nf.max_flow(s, t, ~0u >> 1).c) / n << '\n';
