...
int main() {
    ...
    int n, m, k; cin >> m >> n;
    vec<int> a(m); each (x, a) cin >> x, k += x;
    NetFlow<int> nf(n + m + 2); int s = n + m, t = s + 1;
    r0p (i, m) nf.adde(s, i, a[i]);
    r0p (i, n) nf.adde(i + m, t, 1);
    r0p (i, n) {
        int t, x; cin >> t;
        while (t--) cin >> x, --x, nf.adde(x, i + m, 1);
    }
    if (nf.max_flow(s, t, k) < k) return cout << "No Solution!\n", 0;
    vec<vec<int>> A(m);
    each (e, nf.edges()) if (e.u < m && e.v < n + m && e.f)
        A[e.u].push_back(e.v - m);
    r0p (i, m) {
        cout << i + 1 << ": ";
        each (x, A[i]) cout << x + 1 << ' ';
        cout << '\n';
